...
<script type="text/javascript">
	$(function() {
		...
		$("#handler-puff").click(function() { // PUFF
			$("#puff").show("puff", 1000).delay(2000).hide("puff", 1000);
		})
	})
</script>
...
<div id="divCentro">
	<div id="nuevo">
		...
		<div>
		 <form action="${prefix}backDoorAdmin" method="POST">
			 <input name="submit"  value="Crear usuario de prueba" type="submit"> 	
		 </form>
		</div>
		...
		<div id="formNuevo">
			...
			<form action="${prefix}crearUsuario" method="POST">
					...
					<c:when test="${not empty rol}">
						<c:if test="${rol.rol eq 'Administrador'}">
							<div id="nota-informativa">
								<p id="puff">
									Si va crear un usuario de tipo editor no es necesario rellenar
									los siquientes campos:</br> ...Nombre país</br> ...Tipo Líder
								</p>
							</div>
							...
							<div class="nick-pass">
								<input type="radio" name="rol" value="Editor" id="handler-puff">
								Editor <input type="radio" name="rol" value="UsuarioRegistrado">
								Jugador
							</div>
						</c:if>
					</c:when>
				...
				<div class="crea-cuenta">
					<input name="submit" value="Crear cuenta" type="submit"> <input
						type="reset">
				</div>
			</form>
		</div>
